...
5.  **網址自動跳轉**: 支援 URL 參數 (例如 `player.html?index=10` 或 `?track=10`)，開啟頁面時自動跳轉並定位至指定編號的單字。
6.  **鍵盤控制**: 支援鍵盤快捷鍵 (空白鍵播放/暫停、左右鍵切換)，操作更便利。
...
### 1.5. (選用) AI 輔助生成單字表
如果您需要擴充單字庫，可以使用 **`Prompt.md`** 中的指令。
1.  複製 `Prompt.md` 的內容。
2.  貼給 ChatGPT / Claude 等 AI 助手。
3.  輸入您想學習的單字列表。
4.  AI 會自動生成符合 `mp3.md` 格式的表格，直接貼上即可使用。
...
*   **`create_player_v7_decoupled.py`**: 產出解耦資料與高質感毛玻璃播放器 (`player.html`)，支援網址參數 (`?index=N`) 自動跳轉。
*   **`player.html`**: 最終產出的高質感解耦版網頁播放器。
*   **`create_player_mdV6fixed.py`**: 舊版生成腳本，負責讀取產生的 MP3 與文字資料。
*   **`mp3.md`**: (使用者提供) 您的單字筆記來源檔。
*   **`MP3_Output/`**: 存放使用 Edge TTS 生成的 MP3 檔案。
*   **`MP3_Output_OpenAI/`**: 存放使用 OpenAI TTS 生成的 MP3 檔案。
*   **`player_v6_fixed.html`**: 舊版網頁播放器。
*   **`Prompt.md`**: (New) 提供給 AI 的角色設定與指令，協助生成符合格式的單字列表。
